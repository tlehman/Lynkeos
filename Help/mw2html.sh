...
13/03/2011 Jean-Etienne Lamiaud
      Take localisation into account
      Use ?title= addresses
      Restrict dump to an address space
"""
...
class Config:
  ...
  def __init__(self, rooturl, outdir,
               ...
               remove_png=True, remove_history=True, restrict=None):
...
def monobook_fix_html_sidebar(doc, config):
  ...
  SIDEBAR_ID = 'SIDEBAR' + hashlib.sha1(str(random.random())).hexdigest()
  ...
  if '/math/' in url:
    ...
    if os.path.splitext(tail)[1] == '.png':
      ...
      if set(tail) <= set('0123456789abcdef') and len(tail) == 32:
        ans = 'math_' + hashlib.sha1(tail).hexdigest()[:4] + '.png'
...
def should_follow(rooturl, url, config):
  ...
  # False if out of restricted URL
  if config.restrict is not None and not url.startswith(config.restrict):
   return False
  ...
  try:
    (opts, args) = getopt.gnu_getopt(sys.argv[1:], 'fl:t:b:i:',
                   ['force', 'no-flatten', 'no-lower', 'no-clean',
                    ...
                    'no-remove-png', 'no-remove-history', "restrict="])
